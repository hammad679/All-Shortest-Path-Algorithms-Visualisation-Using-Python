...
class GraphVisualization():
    ...
    #(window,color,starting point(centre),radius,thickness)
                ...
                for index,node in enumerate(inputnodes):
                      ...
                      tempnode = Node(red, node,20,1,index)
...
def Dijkstra(graph, source, window):
     ...
     graph[source].SetColor(window,red)
     ...
     for i in range(len(graph)):
           ...
           if i != source and final_edges[i] != None:
     ...
     Answer(window,black,"Dijkstra: "+str(sum(weights)))
...
def BellmanFord(graph, source, window):
     ...
     Answer(window,black,"BellmanFord")
...
def FloydWarshall(graph, window):
     ...
     Answer(window,black,str("FloydWarshall total cost : ")+str(total_cost))
